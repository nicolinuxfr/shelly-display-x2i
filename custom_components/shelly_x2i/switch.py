...
class ShellyScreenOffWhenIdleSwitch(ShellyX2iBaseEntity, SwitchEntity, RestoreEntity):
    """Whether the display should power off after firmware-defined idle."""

    _attr_translation_key = "screen_off_when_idle"
    ...
    @property
    def is_on(self) -> bool | None:
        """Return whether the device powers off the screen when idle."""
        config = self.coordinator.data.get("config")
    ...
    async def async_turn_on(self, **kwargs) -> None:
        """Enable screen off when idle."""
        self._optimistic_state = True
        self.coordinator.mark_local_action()
        self.async_write_ha_state()
        await self.coordinator.async_set_screen_off_when_idle(True)
        self.coordinator.schedule_refresh()


    async def async_turn_off(self, **kwargs) -> None:
        """Disable screen off when idle."""
        self._optimistic_state = False
        self.coordinator.mark_local_action()
        self.async_write_ha_state()
        await self.coordinator.async_set_screen_off_when_idle(False)
        self.coordinator.schedule_refresh()


class ShellyScreenSaverEnabledSwitch(ShellyX2iBaseEntity, SwitchEntity, RestoreEntity):
    """Whether the display screen saver is enabled."""

    _attr_translation_key = "screen_saver_enabled"
    ...
    @property
    def is_on(self) -> bool | None:
        """Return whether the screen saver is enabled."""
        config = self.coordinator.data.get("config")
        ui = config.get("ui") if isinstance(config, dict) else None
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        self.coordinator.mark_local_action()
        self.async_write_ha_state()
        await self.coordinator.async_set_screen_saver_enabled(True)
        self.coordinator.schedule_refresh()
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        self.coordinator.mark_local_action()
        self.async_write_ha_state()
        await self.coordinator.async_set_screen_saver_enabled(False)
        self.coordinator.schedule_refresh()
